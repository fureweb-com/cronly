...
function assertSupportedPath(absolutePath) {
  if (/[\r\n]/.test(absolutePath)) {
    throw new Error('경로에 줄바꿈 문자를 포함할 수 없어요. crontab에 안전하게 기록할 수 있는 경로만 지원해요.')
  }
}
...
export function parseBlocks(crontabText) {
  ...
  while (i < lines.length) {
    const beginMatch = lines[i].match(
      new RegExp(`^# \\[${PREFIX}:begin\\] id=(\\w+) path=(.+?) runtime=(\\S+)$`)
    )
    if (!beginMatch) {
      i++
      continue
    ...
    const path = beginMatch[2]
    const runtime = beginMatch[3]
    const beginLine = i
